...
export default function OrdersPage() {
  ...
  const fetchCartCount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCartCount(0);
        setCartError(false); // Don't show error, just set count to 0
      }
    } catch (error) {
      console.error('Error fetching cart count:', error);
      setCartCount(0);
      setCartError(false); // Don't show error for unauthenticated users
    } finally {
  ...
  return (
    ...
    <div className="customer-homepage">
      <Header
        cartCount={isCartLoading ? '...' : (username && username !== 'Guest' ? cartCount : 0)}
        username={username}
      />
